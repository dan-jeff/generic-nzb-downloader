...
// IPC Handlers
ipcMain.handle('start-download', async (_event, url, target, filename, providerId) => {
    if (!downloadManager)
        return { success: false, error: 'Download Manager not initialized' };
    try {
        const downloadContent = typeof url === 'string' ? url : Buffer.from(url);
...
ipcMain.handle('delete-download', async (_event, id, removeFiles) => {
    if (!downloadManager)
        return false;
    return await downloadManager.delete(id, removeFiles);
});
ipcMain.handle('delete-download-files', async (_event, id) => {
    if (!downloadManager)
        return false;
...
ipcMain.handle('update-search-settings', (_event, settings) => {
...
ipcMain.handle('update-download-settings', (_event, settings) => {
...
ipcMain.on('set-auto-update', (_event, enable) => {
